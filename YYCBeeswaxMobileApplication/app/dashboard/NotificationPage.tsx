        ...
        if (settingType === 'common') {
          setCommonSetting((prevSettings) =>
            prevSettings.map((setting) =>
              setting.key === key ? { ...setting, toggle: !setting.toggle } : setting
            )
          );
        ...
        <View style={mainStyles.container}>
            ...
            { commonSettings.map((item) => (
                    <View style={notificationPageStyles.item} key={item.key}>
                        ...
                        <Switch style ={notificationPageStyles.itemToggle} onValueChange={() => toggleSwitch(item.key, 'common')}
            value={item.toggle} trackColor={{ false: colors.white, true: colors.yellow}}/>
                    </View>
            ))}
            ...
            { promotionSettings.map((item) => (
                    <View style={notificationPageStyles.item} key={item.key}>
                        <Text style={notificationPageStyles.itemTitle}>{item.name}</Text>
                        <Switch style ={notificationPageStyles.itemToggle} onValueChange={() => toggleSwitch(item.key, 'promotion')}
            value={item.toggle} trackColor={{ false: colors.white, true: colors.yellow }}/>
                    </View>
            ))}
            <Pressable style={notificationPageStyles.button}>
                <Text style={notificationPageStyles.buttonText}>Confirm Changes</Text>
            </Pressable>
        </View>
    )
}